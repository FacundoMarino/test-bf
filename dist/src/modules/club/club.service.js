...
const puntoo_templates_1 = require("../mail/puntoo-templates");
const court_plan_limit_constants_1 = require("./court-plan-limit.constants");
const court_schedule_time_util_1 = require("./court-schedule-time.util");
const FIXED_SERIES_DEFAULT_YEARS = 10;
let ClubService = ClubService_1 = class ClubService {
    prisma;
    supabaseService;
    mailService;
    logger = new common_1.Logger(ClubService_1.name);
    constructor(prisma, supabaseService, mailService) {
        this.prisma = prisma;
        this.supabaseService = supabaseService;
        this.mailService = mailService;
    }
    async resolveEmailsFromAuth(userIds) {
        ...
        const pairs = await Promise.all(userIds.map(async (userId) => {
            try {
                ...
                return [userId, email && email.length > 0 ? email : null];
            }
            catch {
                return [userId, null];
            }
        }));
        return new Map(pairs.filter((p) => typeof p[1] === 'string'));
    }
    async findScheduleRowContainingMinute(courtId, dayOfWeek, minute) {
        ...
        return ((rows ?? []).find((s) => (0, court_schedule_time_util_1.isMinuteWithinSchedule)(minute, s.startTimeMinutes, s.endTimeMinutes)) ?? null);
    }
    assertValidScheduleTimeRangeDto(dto) {
        if (!(0, court_schedule_time_util_1.isValidScheduleTimeRange)(dto.startTimeMinutes, dto.endTimeMinutes)) {
        ...
        if (dto.slotDurationMinutes === undefined ||
            ...
            throw new common_1.BadRequestException('La duración es obligatoria');
        }
    ...
    async resolveSlotPricePerHour(courtId, start) {
        ...
        const utcMinutesFromMidnight = start.getUTCHours() * 60 + start.getUTCMinutes();
        const scheduleForSlot = (await this.findScheduleRowContainingMinute(courtId, localDayOfWeek, localMinutesFromMidnight)) ??
            (await this.findScheduleRowContainingMinute(courtId, utcDayOfWeek, utcMinutesFromMidnight));
    ...
    async computeBookingMoney(params) {
        const hours = (params.end.getTime() - params.start.getTime()) / (60 * 60 * 1000);
        const pricePerHour = await this.resolveSlotPricePerHour(params.courtId, params.start);
    ...
    matchGenderUiFromBooking(manualClubNotes) {
        return this.parseMatchGender(manualClubNotes);
    }
    buildMatchParticipantSlots(params) {
        ...
        for (let i = 0; i < extraSlots; i++) {
            ...
            const struck = params.strikeProfileId !== undefined &&
    ...
    async generateUniqueInviteCode() {
        ...
        throw new common_1.BadRequestException('Unable to generate invite code');
    }
    async assertPlayerCanAccessCourt(clubId, courtId, userId) {
    ...
    async findAll(query) {
        ...
        if (query.courtType)
            where.courtType = query.courtType;
        ...
        const ownerProfiles = ownerIds.length
            ...
            : [];
        const amenitiesByOwnerId = new Map(ownerProfiles.map((p) => [p.id, p.amenities]));
        return {
            data: data.map((club) => ({
                ...club,
                amenities: (club.createdBy
    ...
    async createCourt(clubId, dto) {
        ...
        if (existingCount >= club.courtCount) {
            throw new common_1.BadRequestException(court_plan_limit_constants_1.COURT_PLAN_LIMIT_REACHED_MESSAGE);
        }
    ...
    async findCourts(clubId, query, viewerId) {
        const page = query.page ?? 1;
        const limit = query.limit ?? 20;
        const skip = (page - 1) * limit;
        const club = await this.prisma.club.findUnique({
    ...
    async updateCourt(clubId, courtId, dto) {
        ...
        try {
            ...
            if (dto.lighting !== undefined)
                data.lighting = dto.lighting;
            if (dto.listed !== undefined)
                data.listed = dto.listed;
            ...
            throw new common_1.BadRequestException('Could not update court');
        }
    }
    async removeCourt(clubId, courtId) {
        ...
        if (activeFutureBookings > 0) {
            throw new common_1.BadRequestException('La pista tiene reservas activas');
        }
        try {
            ...
            throw new common_1.BadRequestException('Could not delete court');
        }
    }
    ...
    schedulePeriodsCalendarOverlap(a, b) {
    ...
    schedulesTimeRangesOverlap(a, b) {
        if (a.dayOfWeek !== b.dayOfWeek)
            return false;
        if (!this.schedulePeriodsCalendarOverlap(a, b))
            return false;
        return (0, court_schedule_time_util_1.scheduleTimeRangesOverlap)(a.startTimeMinutes, a.endTimeMinutes, b.startTimeMinutes, b.endTimeMinutes);
    }
    parseMatchGender(raw) {
    ...
    assertSchedulesNoPairwiseOverlap(schedules) {
        for (let i = 0; i < schedules.length; i++) {
            for (let j = i + 1; j < schedules.length; j++) {
                if (this.schedulesTimeRangesOverlap(schedules[i], schedules[j])) {
                    throw new common_1.BadRequestException('El rango horario se solapa con uno existente');
                }
            }
        }
    }
    ...
    scheduleIdentityWithoutPeriodEnd(row) {
        return [
            row.dayOfWeek,
    ...
    assertOnlyPeriodEndEditableWithFutureBookings(currentRows, incomingRows, now) {
        if (currentRows.length !== incomingRows.length) {
            throw new common_1.BadRequestException('Con reservas activas solo podés editar la fecha "Hasta" de horarios existentes.');
        }
        ...
        for (const dto of incomingRows) {
            ...
            if (!matchList || matchList.length === 0) {
                throw new common_1.BadRequestException('Con reservas activas solo podés editar la fecha "Hasta".');
            }
            ...
            if (!incomingEndYmd) {
                throw new common_1.BadRequestException('Con reservas activas, la fecha "Hasta" es obligatoria y debe ser válida.');
            }
            ...
            if (incomingEndDate.getTime() < today.getTime() && !unchangedEnd) {
                throw new common_1.BadRequestException('La fecha "Hasta" debe ser hoy o futura cuando hay reservas activas.');
            }
        }
    }
    bookingCoveredBySchedules(booking, schedules) {
        ...
        const bookingStartMin = booking.start.getHours() * 60 + booking.start.getMinutes();
        const bookingEndMin = booking.end.getHours() * 60 + booking.end.getMinutes();
        for (const row of schedules) {
            ...
            if (row.periodStart && row.periodEnd) {
                ...
                const peEndOfDay = new Date(pe.getFullYear(), pe.getMonth(), pe.getDate(), 23, 59, 59, 999);
                if (bookingDayStart.getTime() < ps.getTime() ||
                    bookingDayStart.getTime() > peEndOfDay.getTime()) {
                    continue;
                }
            }
            if ((0, court_schedule_time_util_1.isIntervalWithinSchedule)(bookingStartMin, bookingEndMin, row.startTimeMinutes, row.endTimeMinutes)) {
                return true;
            }
        }
        return false;
    }
    async replaceCourtSchedules(clubId, courtId, schedules, confirmCancelAffectedBookings = false) {
        ...
        for (const dto of schedules) {
            ...
            if (dto.periodStart && dto.periodEnd) {
                ...
                if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) {
                    throw new common_1.BadRequestException('Fechas de período inválidas');
                }
                if (start.getTime() > end.getTime()) {
                    throw new common_1.BadRequestException('La fecha de inicio del período debe ser anterior o igual al fin');
                }
            }
        }
        ...
        if (futureActive.length > 0) {
            this.assertOnlyPeriodEndEditableWithFutureBookings(existingSchedules, schedules, now);
        }
        await this.prisma.$transaction(async (tx) => {
            await tx.courtSchedule.deleteMany({ where: { courtId } });
            for (const dto of schedules) {
    ...
    async createCourtSchedule(clubId, courtId, dto) {
        ...
        if (dto.periodStart && dto.periodEnd) {
            ...
            const end = new Date(dto.periodEnd);
            if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) {
                throw new common_1.BadRequestException('Invalid period date');
            }
            if (start.getTime() > end.getTime()) {
    ...
    async listCourtSlots(clubId, courtId, query, viewerId) {
        ...
        const targetDayStart = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), 0, 0, 0, 0);
        const targetDayEnd = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), 23, 59, 59, 999);
        ...
        const regularIntervals = this.buildRegularSlotIntervalsFromSchedules(schedules, targetDate);
        const slots = [];
        for (const interval of regularIntervals) {
            const overlapsCustom = customSlotRows.some((c) => this.minutesOverlap(interval.start, interval.end, c.startTimeMinutes, c.endTimeMinutes));
            if (overlapsCustom)
                continue;
            const start = (0, court_schedule_time_util_1.dateFromScheduleAbsoluteMinutes)(targetDate, interval.start);
            const end = (0, court_schedule_time_util_1.dateFromScheduleAbsoluteMinutes)(targetDate, interval.end);
            slots.push({
                start,
                end,
                isAvailable: true,
        ...
        for (const custom of customSlotRows) {
            slots.push({
                start: (0, court_schedule_time_util_1.dateFromScheduleAbsoluteMinutes)(targetDate, custom.startTimeMinutes),
                end: (0, court_schedule_time_util_1.dateFromScheduleAbsoluteMinutes)(targetDate, custom.endTimeMinutes),
                isAvailable: true,
        ...
        for (const slot of slots) {
            ...
            const blockedByException = !slot.isCustom &&
                this.bookingIntervalBlockedByExceptions(exceptions, slotStartMinutes, slotEndMinutes);
            if (overlapping || blockedByException)
    ...
    async listCourtAvailabilityExceptions(clubId, courtId, month) {
        const court = await this.prisma.court.findFirst({
            where: { id: courtId, clubId },
        });
        if (!court)
    ...
    async replaceCourtAvailabilityExceptions(clubId, courtId, month, exceptions) {
        const court = await this.prisma.court.findFirst({
            where: { id: courtId, clubId },
        });
        if (!court)
        ...
        for (const ex of exceptions) {
            if (!ex.isClosedAllDay) {
                if (typeof ex.startTimeMinutes !== 'number' ||
                    typeof ex.endTimeMinutes !== 'number' ||
                    ex.endTimeMinutes <= ex.startTimeMinutes) {
                    throw new common_1.BadRequestException('Invalid exception time range for non all-day exception');
                }
            }
        }
        await this.prisma.$transaction(async (tx) => {
    ...
    async listCourtCustomSlots(clubId, courtId, month) {
        const court = await this.prisma.court.findFirst({
            where: { id: courtId, clubId },
        });
        if (!court)
    ...
    async createCourtCustomSlot(clubId, courtId, userId, dto) {
        ...
        const targetDayEnd = new Date(calendarDay.getFullYear(), calendarDay.getMonth(), calendarDay.getDate(), 23, 59, 59, 999);
        ...
        const mergedExceptions = this.mergeCancelledSlotsIntoExceptions(exceptions, dto.cancelledSlots);
        if (mergedExceptions.some((e) => e.isClosedAllDay)) {
            throw new common_1.BadRequestException('El día está cerrado por completo');
        }
        this.assertCustomSlotDoesNotOverlapActiveSlots(dto.startTimeMinutes, dto.endTimeMinutes, this.buildRegularSlotIntervalsFromSchedules(schedules, calendarDay), mergedExceptions, existingCustom);
        return this.prisma.$transaction(async (tx) => {
            for (const range of dto.cancelledSlots ?? []) {
                if (range.endTimeMinutes <= range.startTimeMinutes)
                    continue;
    ...
    async cancelTentativePublicMatchesOverlapping(tx, courtId, start, end) {
    ...
    combineLocalDateAndMinutes(calendarDay, minutesFromMidnight) {
    ...
    async getCourtSchedulesForCalendarDay(courtId, calendarDay) {
        const targetDayStart = new Date(calendarDay.getFullYear(), calendarDay.getMonth(), calendarDay.getDate(), 0, 0, 0, 0);
        const targetDayEnd = new Date(calendarDay.getFullYear(), calendarDay.getMonth(), calendarDay.getDate(), 23, 59, 59, 999);
        const dayOfWeek = calendarDay.getDay();
        const rows = await this.prisma.courtSchedule.findMany({
            where: {
    ...
    pickScheduleContainingInterval(schedules, intervalStartMin, intervalEndMin) {
        for (const s of schedules) {
            if ((0, court_schedule_time_util_1.isIntervalWithinSchedule)(intervalStartMin, intervalEndMin, s.startTimeMinutes, s.endTimeMinutes)) {
                return s;
            }
        }
        return null;
    }
    bookingIntervalBlockedByExceptions(exceptions, bookingStartMin, bookingEndMin) {
        if (exceptions.some((e) => e.isClosedAllDay))
            return true;
        return exceptions.some((e) => {
            if (typeof e.startTimeMinutes !== 'number' ||
                typeof e.endTimeMinutes !== 'number') {
                return false;
            }
            return (e.startTimeMinutes < bookingEndMin && e.endTimeMinutes > bookingStartMin);
        });
    }
    minutesOverlap(aStart, aEnd, bStart, bEnd) {
        return aStart < bEnd && aEnd > bStart;
    }
    buildRegularSlotIntervalsFromSchedules(schedules, targetDate) {
        const generatedByKey = new Map();
        const sortedRows = [...schedules].sort((a, b) => a.startTimeMinutes - b.startTimeMinutes);
        for (const schedule of sortedRows) {
            let current = schedule.startTimeMinutes;
            const pricePerHour = schedule.pricePerHour ?? 0;
            const effectiveEnd = (0, court_schedule_time_util_1.effectiveEndTimeMinutes)(schedule.startTimeMinutes, schedule.endTimeMinutes);
    ...
    mergeCancelledSlotsIntoExceptions(dbExceptions, clientCancelled) {
        const merged = dbExceptions.map((e) => ({ ...e }));
        for (const range of clientCancelled ?? []) {
            const dup = merged.some((e) => !e.isClosedAllDay &&
    ...
    assertCustomSlotDoesNotOverlapActiveSlots(customStart, customEnd, regularIntervals, exceptions, existingCustom) {
        for (const interval of regularIntervals) {
            const cancelled = this.bookingIntervalBlockedByExceptions(exceptions, interval.start, interval.end);
            if (cancelled)
                continue;
            if (this.minutesOverlap(customStart, customEnd, interval.start, interval.end)) {
                throw new common_1.BadRequestException(`El horario se superpone con el turno ${this.formatMinutesLabel(interval.start)}-${this.formatMinutesLabel(interval.end)} que no está cancelado. Cancelalo primero o guardá las excepciones con "Listo".`);
            }
        }
        for (const custom of existingCustom) {
            if (this.minutesOverlap(customStart, customEnd, custom.startTimeMinutes, custom.endTimeMinutes)) {
                throw new common_1.BadRequestException('El horario se superpone con otro turno personalizado');
            }
        }
    }
    async findCustomSlotAtStart(courtId, targetDayStart, startMinutes) {
    ...
    intervalsOverlapDateRange(aStart, aEnd, bStart, bEnd) {
        return aStart < bEnd && aEnd > bStart;
    }
    localDateKeyFromDate(d) {
        return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
    }
    filterSchedulesActiveOnCalendarDay(rows, calendarDay) {
        const targetDayStart = new Date(calendarDay.getFullYear(), calendarDay.getMonth(), calendarDay.getDate(), 0, 0, 0, 0);
        const targetDayEnd = new Date(calendarDay.getFullYear(), calendarDay.getMonth(), calendarDay.getDate(), 23, 59, 59, 999);
        ...
        return rows
            ...
            const ps = new Date(row.periodStart);
            const pe = new Date(row.periodEnd);
            return (ps.getTime() <= targetDayEnd.getTime() &&
    ...
    async createCourtBooking(clubId, courtId, userId, dto) {
        ...
        const manualGuestsRequested = dto.manualGuests !== null && dto.manualGuests !== undefined;
        const wantsManualClubFields = manualGuestsRequested || manualNotesTrimmed.length > 0;
        if (wantsManualClubFields) {
            await this.assertClubOwner(clubId, userId);
        }
        if (manualGuestsRequested && manualGuestsInput.length === 0) {
            throw new common_1.BadRequestException('Indicá al menos un jugador con nombre para la reserva manual');
        }
        const start = new Date(dto.start);
        if (Number.isNaN(start.getTime())) {
            throw new common_1.BadRequestException('Invalid start datetime');
        }
        const minutesFromMidnight = start.getHours() * 60 + start.getMinutes();
        const calendarDay = new Date(start.getFullYear(), start.getMonth(), start.getDate());
        const targetDayStart = new Date(start.getFullYear(), start.getMonth(), start.getDate(), 0, 0, 0, 0);
        const customSlotAtStart = await this.findCustomSlotAtStart(courtId, targetDayStart, minutesFromMidnight);
        const schedules = await this.getCourtSchedulesForCalendarDay(courtId, start);
        let endMinutes;
        let scheduleForBooking;
        if (customSlotAtStart) {
            ...
            if (dto.durationMinutes !== undefined &&
                ...
                throw new common_1.BadRequestException('La duración no coincide con el turno personalizado');
            }
            scheduleForBooking =
                this.pickScheduleContainingInterval(schedules, minutesFromMidnight, endMinutes) ?? schedules[0] ?? null;
        }
            ...
            if (dm < 1 || dm > 24 * 60) {
                throw new common_1.BadRequestException('Invalid durationMinutes');
            }
            endMinutes = minutesFromMidnight + dm;
            if (endMinutes > 24 * 60) {
                throw new common_1.BadRequestException('Booking cannot cross midnight');
            }
            scheduleForBooking = this.pickScheduleContainingInterval(schedules, minutesFromMidnight, endMinutes);
        }
        else {
            const schedule = schedules.find((s) => (0, court_schedule_time_util_1.isMinuteWithinSchedule)(minutesFromMidnight, s.startTimeMinutes, s.endTimeMinutes));
            if (!schedule) {
                throw new common_1.BadRequestException('No schedule for this court at the given time');
            }
            const slotOffset = minutesFromMidnight - schedule.startTimeMinutes;
            if (slotOffset % schedule.slotDurationMinutes !== 0) {
                throw new common_1.BadRequestException('Start time is not aligned with slot duration');
            }
            endMinutes = minutesFromMidnight + schedule.slotDurationMinutes;
            const scheduleEffectiveEnd = (0, court_schedule_time_util_1.effectiveEndTimeMinutes)(schedule.startTimeMinutes, schedule.endTimeMinutes);
            if (endMinutes > scheduleEffectiveEnd) {
                throw new common_1.BadRequestException('Slot exceeds schedule end time');
            }
        ...
        const end = (0, court_schedule_time_util_1.dateFromScheduleAbsoluteMinutes)(calendarDay, endMinutes);
        const exceptions = await this.prisma.courtScheduleException.findMany({
            where: { courtId, date: targetDayStart },
        });
        if (!customSlotAtStart &&
            this.bookingIntervalBlockedByExceptions(exceptions, minutesFromMidnight, endMinutes)) {
            throw new common_1.BadRequestException('Court availability is blocked at this time');
        }
        const inviteCode = await this.generateUniqueInviteCode();
        const booking = await this.prisma.$transaction(async (tx) => {
            await this.cancelTentativePublicMatchesOverlapping(tx, courtId, start, end);
            ...
            return tx.courtBooking.create({
                data: {
                    ...
                    ...(manualGuestsInput.length > 0
                        ? {
                            manualGuests: manualGuestsInput,
                            manualClubNotes: manualNotesTrimmed.length > 0 ? manualNotesTrimmed : null,
                        }
    ...
    async createFixedSeriesBookings(clubId, courtId, userId, dto) {
        await this.assertClubOwner(clubId, userId);
        const court = await this.prisma.court.findFirst({
            where: { id: courtId, clubId },
        });
        ...
        if (startDay.getDay() !== dto.dayOfWeek) {
            throw new common_1.BadRequestException('startDate must be the same weekday as dayOfWeek');
        }
        let endDay;
        if (dto.endDate?.trim()) {
        ...
        if (endMin > 24 * 60) {
            throw new common_1.BadRequestException('Fixed series occurrence cannot cross midnight');
        }
        const allScheduleRows = await this.prisma.courtSchedule.findMany({
            where: { courtId, dayOfWeek: dto.dayOfWeek },
        });
        const excRangeStart = new Date(startDay.getFullYear(), startDay.getMonth(), startDay.getDate());
        const excRangeEnd = new Date(endDay.getFullYear(), endDay.getMonth(), endDay.getDate());
        ...
        const occurrences = [];
        const skippedNoScheduleDates = [];
        for (let d = new Date(startDay); d.getTime() <= endDay.getTime(); d.setDate(d.getDate() + 7)) {
            const schedules = this.filterSchedulesActiveOnCalendarDay(allScheduleRows, d);
            const schedule = this.pickScheduleContainingInterval(schedules, startMin, endMin);
            if (!schedule) {
            ...
            if (this.bookingIntervalBlockedByExceptions(exceptions, startMin, endMin)) {
                throw new common_1.BadRequestException(`Court has an availability exception on ${dk}`);
            }
            occurrences.push({
        ...
        if (occurrences.length === 0) {
            ...
            throw new common_1.BadRequestException(`No schedule covers this interval (${startLabel}-${endLabel}) in the requested date range`);
        }
        ...
        const overlapsOcc = (b) => occurrences.some((o) => this.intervalsOverlapDateRange(b.start, b.end, o.start, o.end));
        const blocking = existingInRange.filter((b) => this.bookingOccupiesSlotRow(b) && overlapsOcc(b));
        const removable = existingInRange.filter((b) => !this.bookingOccupiesSlotRow(b) && overlapsOcc(b));
        const guestNameTrim = dto.guestName?.trim() ?? '';
        const guestPhoneTrim = dto.guestPhone?.trim() ?? '';
        const notesTrim = dto.notes?.trim() ?? '';
        const manualGuestsForSeries = guestNameTrim.length > 0
            ? [{ name: guestNameTrim, phone: guestPhoneTrim || null }]
            : [];
        if (guestNameTrim.length > 200) {
            throw new common_1.BadRequestException('Nombre de jugador demasiado largo');
        }
        if (guestPhoneTrim.length > 40) {
            throw new common_1.BadRequestException('Teléfono demasiado largo');
        }
        const rulePayload = {
        ...
        if (blocking.length > 0) {
            throw new common_1.ConflictException({
                message: 'Hay reservas que ocupan slot en el horario del turno fijo; no se puede crear.',
                blockingBookingIds: blocking.map((b) => b.id),
            });
        }
        if (removable.length > 0 && dto.confirmRemoveOverlapping !== true) {
            throw new common_1.BadRequestException({
                message: 'Hay turnos superpuestos que se pueden eliminar; confirmá con confirmRemoveOverlapping.',
                removableBookingIds: removable.map((b) => b.id),
                requiresConfirmation: true,
            });
        }
        const seriesId = (0, crypto_1.randomUUID)();
            ...
            for (let i = 0; i < occurrences.length; i += batchSize) {
                ...
                await tx.courtBooking.createMany({
                    data: slice.map((occ, j) => ({
                        ...
                        fixedSeriesRule: rulePayload,
                        inviteCode: null,
                        ...(manualGuestsForSeries.length > 0
                            ? {
                                manualGuests: manualGuestsForSeries,
                            }
                            : {}),
    ...
    parsePricePerHourMap(pricing) {
        ...
        for (const row of pricing) {
            if (row &&
                ...
                const p = Number(row.pricePerHour);
                if (!Number.isNaN(p))
                    m.set(day, p);
            }
        }
    ...
    hourPriceForInstant(priceMap, instant, fallback) {
        const keys = [
            'sunday',
            'monday',
            'tuesday',
    ...
    sumAvailableMinutesForClub(courts, periodStart, periodEnd) {
        let total = 0;
        for (const court of courts) {
            total += this.availableMinutesForCourt(court.schedules, periodStart, periodEnd);
        }
        return total;
    }
    availableMinutesForCourt(schedules, periodStart, periodEnd) {
        let total = 0;
        ...
        for (let t = periodStart.getTime(); t < periodEnd.getTime(); t += dayMs) {
            ...
            for (const s of schedules) {
                if (s.dayOfWeek === dow) {
                    total += (0, court_schedule_time_util_1.scheduleDurationMinutes)(s.startTimeMinutes, s.endTimeMinutes);
                }
            }
        }
        return total;
    }
    sumAvailableSlotsForClub(courts, periodStart, periodEnd) {
        let total = 0;
        for (const court of courts) {
            total += this.availableSlotsForCourt(court.schedules, periodStart, periodEnd);
        }
        return total;
    }
    availableSlotsForCourt(schedules, periodStart, periodEnd) {
        let total = 0;
        const dayMs = 24 * 60 * 60 * 1000;
        for (let t = periodStart.getTime(); t < periodEnd.getTime(); t += dayMs) {
            const d = new Date(t);
            ...
            for (const s of schedules) {
                if (s.dayOfWeek === dow) {
                    const scheduleMinutes = (0, court_schedule_time_util_1.scheduleDurationMinutes)(s.startTimeMinutes, s.endTimeMinutes);
                    const slotMinutes = Number.isFinite(s.slotDurationMinutes) && s.slotDurationMinutes > 0
    ...
    overlapMs(bookingStart, bookingEnd, windowStart, windowEnd) {
    ...
    scheduleHourPriceForInstant(schedules, instant) {
        if (!schedules?.length)
            return null;
        ...
        const hasValidRate = (s) => Number.isFinite(s.pricePerHour) && s.pricePerHour > 0;
        const match = schedules.find((s) => hasValidRate(s) &&
            s.dayOfWeek === localDow &&
            s.startTimeMinutes <= localMinute &&
            s.endTimeMinutes > localMinute) ??
            schedules.find((s) => hasValidRate(s) &&
                s.dayOfWeek === utcDow &&
                s.startTimeMinutes <= utcMinute &&
                s.endTimeMinutes > utcMinute);
        return match?.pricePerHour ?? null;
    }
    confirmedBookingRevenueDetailsInWindow(bookingStart, bookingEnd, priceMap, defaultHourPrice, courtSchedules, windowStart, windowEnd) {
        const byWeekday = [0, 0, 0, 0, 0, 0, 0];
        const i0 = Math.max(bookingStart.getTime(), windowStart.getTime());
        const i1 = Math.min(bookingEnd.getTime(), windowEnd.getTime());
        if (i0 >= i1)
            return { total: 0, byWeekday };
        let total = 0;
        let cursor = i0;
        const dayMs = 24 * 60 * 60 * 1000;
        while (cursor < i1) {
            ...
            const slotRate = this.scheduleHourPriceForInstant(courtSchedules, cursorDate);
            const rate = slotRate ??
    ...
    confirmedBookingRevenueInWindow(bookingStart, bookingEnd, priceMap, defaultHourPrice, courtSchedules, windowStart, windowEnd) {
        return this.confirmedBookingRevenueDetailsInWindow(bookingStart, bookingEnd, priceMap, defaultHourPrice, courtSchedules, windowStart, windowEnd).total;
    }
    getAnalyticsWindows(range) {
        const now = new Date();
        const dayMs = 24 * 60 * 60 * 1000;
        let periodStart;
        let periodEnd;
    ...
    addBookedMinutesToPeakBuckets(bookingStart, bookingEnd, windowStart, windowEnd, acc) {
        const i0 = Math.max(bookingStart.getTime(), windowStart.getTime());
        const i1 = Math.min(bookingEnd.getTime(), windowEnd.getTime());
        if (i0 >= i1)
            return;
    ...
    sumAvailableMinutesInPeakBuckets(courts, periodStart, periodEnd) {
    ...
    parseManualGuestsForCreate(raw) {
        if (raw == null)
            return [];
        if (!Array.isArray(raw)) {
            throw new common_1.BadRequestException('manualGuests debe ser un arreglo');
        }
        const out = [];
    ...
    async assertClubOwner(clubId, userId) {
        const club = await this.prisma.club.findUnique({ where: { id: clubId } });
        if (!club)
            throw new common_1.NotFoundException('Club not found');
        if (club.createdBy !== userId) {
            throw new common_1.ForbiddenException('You are not the owner of this club');
        }
    }
    async approveCourtBooking(clubId, bookingId, userId) {
        await this.assertClubOwner(clubId, userId);
        ...
        if (!court || court.clubId !== clubId) {
            throw new common_1.NotFoundException('Booking not found for this club');
        }
    ...
    async rejectCourtBooking(clubId, bookingId, userId) {
        await this.assertClubOwner(clubId, userId);
        const booking = await this.prisma.courtBooking.findUnique({
            where: { id: bookingId },
        });
    ...
    async cancelCourtBooking(clubId, bookingId, userId) {
        const booking = await this.prisma.courtBooking.findUnique({
            where: { id: bookingId },
            include: { court: true },
        });
        ...
        const now = new Date();
        if (booking.end <= now) {
            throw new common_1.BadRequestException('No se pueden cancelar reservas que ya han finalizado');
        }
        const club = await this.prisma.club.findUnique({
            where: { id: clubId },
            select: { createdBy: true },
    ...
    async listClubBookings(clubId, userId, query) {
        await this.assertClubOwner(clubId, userId);
        const page = query.page ?? 1;
        ...
        const authEmailMap = ownerIdsWithoutEmail.length > 0
    ...
    async getClubDashboard(clubId, userId, query) {
        await this.assertClubOwner(clubId, userId);
        const range = query.range ?? 'today';
        const { periodStart, periodEnd, prevPeriodStart, prevPeriodEnd } = this.getDashboardWindows(range);
        const club = await this.prisma.club.findUnique({
            where: { id: clubId },
            include: {
        ...
        const scheduleFallbackHourPrice = club.courts
            .flatMap((c) => c.schedules)
            .find((s) => typeof s.pricePerHour === 'number' &&
            Number.isFinite(s.pricePerHour) &&
            s.pricePerHour > 0)?.pricePerHour ?? 0;
        const defaultHourPrice = priceMap.get('monday') ??
            priceMap.get('saturday') ??
            scheduleFallbackHourPrice;
        const courtSchedulesById = new Map(club.courts.map((c) => [c.id, c.schedules]));
        const availableCurrentSlots = this.sumAvailableSlotsForClub(club.courts, periodStart, periodEnd);
        const availablePrevSlots = this.sumAvailableSlotsForClub(club.courts, prevPeriodStart, prevPeriodEnd);
        ...
        for (const b of metricsBookings) {
            ...
            if (oc > 0) {
                ...
                revenueCurrent += this.confirmedBookingRevenueInWindow(b.start, b.end, priceMap, defaultHourPrice, courtSchedulesById.get(b.courtId), periodStart, periodEnd);
            }
            if (op > 0) {
                bookingsPrev += 1;
                if (this.bookingOccupiesSlotRow(b)) {
                ...
                revenuePrev += this.confirmedBookingRevenueInWindow(b.start, b.end, priceMap, defaultHourPrice, courtSchedulesById.get(b.courtId), prevPeriodStart, prevPeriodEnd);
            }
        }
        const occCurrent = availableCurrentSlots > 0
            ? Math.min(100, (occupancyBookingsCurrent / availableCurrentSlots) * 100)
            : 0;
        const occPrev = availablePrevSlots > 0
        ...
        const nextOwnerIds = Array.from(new Set([...firstBookingByCourtId.values()].map((b) => b.userId)));
        ...
        const courtsSorted = [...club.courts].sort((a, b) => a.name.localeCompare(b.name, 'es'));
            ...
            return {
                ...
                booking: booking
                    ? {
                        ...
                        priceEUR: Math.round(this.confirmedBookingRevenueInWindow(booking.start, booking.end, priceMap, defaultHourPrice, courtSchedulesById.get(booking.courtId), booking.start, booking.end) * 100) / 100,
                    }
                    : null,
            };
        });
        ...
        const openMatchProfileIds = new Set();
        ...
        const openMatchProfiles = openMatchProfileIds.size
            ...
            : [];
        const openMatchProfileMap = new Map(openMatchProfiles.map((p) => [p.id, p]));
        ...
        const openMatches = openMatchRows
            ...
            const organizerIn = b.participants.some((p) => p.profileId === b.userId);
            const filled = Math.min(maxP, b.participants.length + (organizerIn ? 0 : 1));
            ...
            const ordered = [...participantRows].sort((a, c) => {
                if (a.profileId === b.userId)
                    return -1;
                if (c.profileId === b.userId)
                    return 1;
                return 0;
            });
    ...
    async getClubAnalytics(clubId, userId, query) {
        await this.assertClubOwner(clubId, userId);
        const range = query.range ?? 'month';
        const { periodStart, periodEnd, prevPeriodStart, prevPeriodEnd } = this.getAnalyticsWindows(range);
        const club = await this.prisma.club.findUnique({
            where: { id: clubId },
        ...
        const totalSlotsCurrent = this.sumAvailableSlotsForClub(club.courts, periodStart, periodEnd);
        const totalSlotsPrev = this.sumAvailableSlotsForClub(club.courts, prevPeriodStart, prevPeriodEnd);
        const occByCourt = new Map();
        ...
        for (const b of bookings) {
            ...
            if (oc > 0) {
                ...
                if (occupies) {
                    ...
                    if (courtRow)
                        courtRow.bookings += 1;
                    const bookingStartMinUtc = b.start.getUTCHours() * 60 + b.start.getUTCMinutes();
                    const peakBucketIndex = this.peakHourBucketDefs.findIndex((bucket) => bookingStartMinUtc >= bucket.startMin &&
                        bookingStartMinUtc < bucket.endMin);
                    if (peakBucketIndex >= 0)
                        peakBookingCounts[peakBucketIndex] += 1;
                    const revenueDet = this.confirmedBookingRevenueDetailsInWindow(b.start, b.end, priceMap, defaultHourPrice, courtSchedulesById.get(b.courtId), periodStart, periodEnd);
                    if (revenueDet) {
        ...
        const occupancyAvg = totalSlotsCurrent > 0
            ? Math.min(100, Math.round((bookingsCurrent / totalSlotsCurrent) * 1000) / 10)
            : 0;
        const occupancyAvgPrev = totalSlotsPrev > 0
            ? Math.min(100, Math.round((bookingsPrev / totalSlotsPrev) * 1000) / 10)
            : 0;
        const comparisonLabel = range === 'week'
            ? 'vs. semana anterior'
            : range === 'month'
                ? 'vs. mes anterior'
                : 'vs. año anterior';
        const peakHours = this.peakHourBucketDefs.map((def, i) => ({
            label: def.label,
            occupancyPercent: totalSlotsCurrent > 0
                ? Math.min(100, Math.round((peakBookingCounts[i] / totalSlotsCurrent) * 1000) /
                    10)
                : 0,
        }));
        ...
        return {
            ...
            summary: {
                ...
                occupancyChangePercent: this.trendPercent(occupancyAvg, occupancyAvgPrev),
            },
            revenueByWeekday,
            occupancyByCourt,
            peakHours,
    ...
    async createMatch(clubId, userId, dto) {
        const court = await this.prisma.court.findFirst({
        ...
        const minutesFromMidnight = start.getHours() * 60 + start.getMinutes();
        const schedule = await this.findScheduleRowContainingMinute(dto.courtId, dayOfWeek, minutesFromMidnight);
        if (!schedule) {
            throw new common_1.BadRequestException('No schedule for this court at the given time');
        }
        const slotOffset = minutesFromMidnight - schedule.startTimeMinutes;
        ...
        if (isPrivate) {
            const booking = await this.prisma.$transaction(async (tx) => {
                await this.cancelTentativePublicMatchesOverlapping(tx, dto.courtId, start, end);
                const overlapping = await tx.courtBooking.findFirst({
                    where: {
                        courtId: dto.courtId,
                        start: { lt: end },
    ...
    async findMatches(clubId, query) {
        ...
        const orConditions = [];
        if (query.search) {
            orConditions.push({ title: { contains: query.search, mode: 'insensitive' } }, {
                court: {
                    name: { contains: query.search, mode: 'insensitive' },
                },
            }, {
    ...
    async findMyMatches(userId, query) {
        const page = query.page ?? 1;
        const limit = query.limit ?? 20;
        const skip = (page - 1) * limit;
        const orConditions = [
    ...
    async findPublicMatches(userId, query) {
        const page = query.page ?? 1;
        const limit = query.limit ?? 20;
        const skip = (page - 1) * limit;
        const forYou = query.forYou === true;
        let viewerCityForClub;
        if (forYou) {
        ...
        const clubInCourtFilter = {
            approvalStatus: 'APPROVED',
            ...(forYou
                ? {
                    location: {
                        equals: viewerCityForClub,
                        mode: 'insensitive',
                    },
                }
                : {}),
        };
        const courtFilter = query.courtType
            ? {
                type: query.courtType,
    ...
    async joinMatch(matchId, userId) {
        ...
        const now = new Date();
        if (match.end < now) {
            throw new common_1.BadRequestException('Match already finished');
        }
        if (match.userId === userId) {
            throw new common_1.BadRequestException('You are already the match owner');
        }
        const existing = match.participants.find((p) => p.profileId === userId);
        if (existing) {
            throw new common_1.BadRequestException('You are already joined to this match');
        }
        await this.prisma.$transaction(async (tx) => {
            ...
            if (fresh.participants.length >= maxPlayers) {
                throw new common_1.BadRequestException('Match is already full');
            }
    ...
    async leaveMatch(matchId, userId) {
        ...
        if (match.isMatch !== true) {
            throw new common_1.BadRequestException('Booking is not a match');
        }
        if (match.userId === userId) {
            throw new common_1.BadRequestException('Organizer cannot cancel participation');
        }
        const maxPlayersBefore = match.maxPlayers ?? 4;
        const wasFullConfirmed = match.visibility === 'public' &&
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            const becameTentativeAgain = after.visibility === 'public' &&
        ...
        void this.notifyMatchLeaveMail(match, userId, wasFullConfirmed).catch((err) => {
            this.logger.warn(`notifyMatchLeaveMail: ${String(err)}`);
        });
        return { message: 'Participation cancelled' };
    }
    async createLooseMatch(profileId, dto) {
        const inviteCode = await this.generateUniqueInviteCode();
    ...
    async joinLooseMatch(matchId, userId) {
        const loose = await this.prisma.looseMatch.findUnique({
            where: { id: matchId },
            include: { participants: true },
        });
        if (!loose) {
            throw new common_1.NotFoundException('Loose match not found');
        }
        const maxPlayers = 4;
        const currentCount = loose.participants.length;
        if (currentCount >= maxPlayers) {
            throw new common_1.BadRequestException('Loose match is already full');
        }
        const already = loose.participants.some((p) => p.profileId === userId);
        if (already) {
            throw new common_1.BadRequestException('You are already joined');
        }
    ...
    async leaveLooseMatch(matchId, userId) {
        const loose = await this.prisma.looseMatch.findUnique({
            where: { id: matchId },
            include: { participants: true },
        });
    ...
    async getBookingInviteCode(bookingId, userId) {
        ...
        if (!booking)
            return null;
        const isParticipantOrOrganizer = booking.userId === userId ||
            booking.participants.some((p) => p.profileId === userId);
        const isPublicMatchShare = booking.isMatch === true && booking.visibility === 'public';
        const isAuthorized = isParticipantOrOrganizer || isPublicMatchShare;
        if (!isAuthorized) {
            throw new common_1.ForbiddenException('You cannot access this invite code');
        }
        if (booking.inviteCode)
            return booking.inviteCode;
        const inviteCode = await this.generateUniqueInviteCode();
    ...
    async resolveInvite(inviteCode, userId) {
        ...
        if (match) {
            ...
            const isParticipant = match.participants.some((p) => p.profileId === userId);
            const nonOrganizerParticipants = match.participants.filter((p) => p.profileId !== organizerId).length;
            return {
                type: 'match',
                matchId: match.id,
                title: match.title,
        ...
        if (loose) {
            ...
            const isOrganizer = organizerId === userId;
            const isParticipant = loose.participants.some((p) => p.profileId === userId);
            const nonOrganizerParticipants = loose.participants.filter((p) => p.profileId !== organizerId).length;
            return {
                type: 'loose',
                matchId: loose.id,
                title: loose.title,
    ...
    async joinInvite(inviteCode, userId) {
        const match = await this.prisma.courtBooking.findFirst({
            where: { inviteCode },
            select: { id: true },
        });
    ...
    async findMyLooseMatches(profileId, query) {
        const page = query.page ?? 1;
        const limit = query.limit ?? 20;
        const skip = (page - 1) * limit;
        const where = {
    ...
    async findLooseMatches(userId, query) {
        const page = query.page ?? 1;
        const limit = query.limit ?? 20;
        const skip = (page - 1) * limit;
        const sameCity = query.sameCity === true;
        const loosePublicBase = {
            profileId: { not: userId },
            participants: { none: { profileId: userId } },
        };
        let looseOrganizerScope = loosePublicBase;
        if (sameCity) {
            const viewer = await this.prisma.profile.findUnique({
                where: { id: userId },
                select: { location: true },
            ...
            if (!viewerLoc) {
                ...
                const sameCityRows = await this.prisma.$queryRaw `
    ...
    async findMatchDetail(matchId, userId) {
        const booking = await this.prisma.courtBooking.findFirst({
            where: { id: matchId },
            include: {
                court: { include: { club: true } },
        ...
        const isParticipant = booking.participants.some((p) => p.profileId === userId);
        if (booking.visibility !== 'public' && !isOrganizer && !isParticipant) {
            throw new common_1.ForbiddenException('You cannot view this match');
        }
        const organizerProfile = await this.prisma.profile.findUnique({
            where: { id: booking.userId },
        });
        const participantRows = booking.participants.filter((p) => p.profileId !== booking.userId);
        const manualGuestList = this.parseManualBookingGuests(booking.manualGuests);
        const startDate = booking.start;
        const localDayOfWeek = startDate.getDay();
        const utcDayOfWeek = startDate.getUTCDay();
        const localMinutesFromMidnight = startDate.getHours() * 60 + startDate.getMinutes();
        const utcMinutesFromMidnight = startDate.getUTCHours() * 60 + startDate.getUTCMinutes();
        const scheduleForSlot = (await this.prisma.courtSchedule.findFirst({
            where: {
        ...
        return {
            ...
            participantsCount: participantRows.length + 1,
        };
    }
    async findLooseMatchDetail(matchId) {
        ...
        return {
            ...
            participantsCount: 1 + nonOrganizerParticipants.length,
        };
    }
    mapBoardAuthorProfile(p) {
        return {
            id: p.id,
            fullName: p.fullName,
    ...
    async assertCourtBookingBoardAccess(bookingId, userId) {
        const booking = await this.prisma.courtBooking.findFirst({
            where: { id: bookingId },
        ...
        if (!isOrganizer && !isParticipant) {
            throw new common_1.ForbiddenException('Only participants can access the board');
        }
    }
    async assertLooseMatchBoardAccess(looseMatchId, userId) {
        const loose = await this.prisma.looseMatch.findUnique({
    ...
    async listCourtBookingBoardMessages(matchId, userId) {
    ...
    async postCourtBookingBoardMessage(matchId, userId, body) {
    ...
    async deleteCourtBookingBoardMessage(matchId, messageId, userId) {
        await this.assertCourtBookingBoardAccess(matchId, userId);
    ...
    async listLooseMatchBoardMessages(matchId, userId) {
    ...
    async postLooseMatchBoardMessage(matchId, userId, body) {
        const trimmed = body.trim();
        if (!trimmed) {
            throw new common_1.BadRequestException('Message cannot be empty');
        }
    ...
    async deleteLooseMatchBoardMessage(matchId, messageId, userId) {
        await this.assertLooseMatchBoardAccess(matchId, userId);
    ...
    async notifyClosedCourtBookingMail(bookingId) {
        const booking = await this.prisma.courtBooking.findUnique({
            where: { id: bookingId },
            include: { court: { include: { club: true } } },
        });
        if (!booking || booking.isMatch)
            return;
        ...
        const pack = (0, puntoo_templates_1.closedBookingConfirmationEmail)({
        ...
        await this.mailService.sendTransactional({
            ...
            text: pack.text,
            html: pack.html,
            eventType: mail_constants_1.PuntooMailEvent.CLOSED_BOOKING_CONFIRMED,
        });
    }
    async notifyOpenMatchPublishedMail(bookingId) {
        const booking = await this.prisma.courtBooking.findUnique({
            where: { id: bookingId },
            include: {
                court: { include: { club: true } },
        ...
        if (!booking || !booking.isMatch || booking.visibility !== 'public')
            return;
        const emails = await this.resolveEmailsFromAuth([booking.userId]);
        const to = emails.get(booking.userId);
        if (!to)
        ...
        const statusLabel = (0, puntoo_templates_1.bookingStatusLabel)(booking.status, booking.occupiesSlot);
        const modality = (0, puntoo_templates_1.matchGenderLabel)(this.matchGenderUiFromBooking(booking.manualClubNotes));
        const pack = (0, puntoo_templates_1.openMatchPublishedEmail)({
        ...
        await this.mailService.sendTransactional({
            ...
            text: pack.text,
            html: pack.html,
            eventType: mail_constants_1.PuntooMailEvent.OPEN_MATCH_PUBLISHED,
        });
    }
    async notifyMatchJoinMail(matchId, joinedUserId) {
        const booking = await this.prisma.courtBooking.findUnique({
            where: { id: matchId },
            include: {
                court: { include: { club: true } },
        ...
        const fullNow = booking.participants.length >= maxPlayers &&
        ...
        const statusLabel = (0, puntoo_templates_1.bookingStatusLabel)(booking.status, booking.occupiesSlot);
        if (fullNow) {
            const pack = (0, puntoo_templates_1.matchConfirmedAllEmail)({
                statusLabel: (0, puntoo_templates_1.bookingStatusLabel)('CONFIRMED', true),
                level: booking.level,
                modality,
                organizerName,
                start: booking.start,
            ...
            for (const pid of participantIds) {
                ...
                await this.mailService.sendTransactional({
                    ...
                    text: pack.text,
                    html: pack.html,
                    eventType: mail_constants_1.PuntooMailEvent.OPEN_MATCH_CONFIRMED_ALL,
                });
            }
            return;
        }
        ...
        if (orgMail) {
            const orgPack = (0, puntoo_templates_1.playerJoinedOrganizerEmail)({
                statusLabel,
                level: booking.level,
                modality,
                organizerName,
            ...
            await this.mailService.sendTransactional({
                ...
                text: orgPack.text,
                html: orgPack.html,
                eventType: mail_constants_1.PuntooMailEvent.OPEN_MATCH_PLAYER_JOINED_ORGANIZER,
            });
        }
        const joinMail = emailMap.get(joinedUserId);
        if (joinMail) {
            const selfPack = (0, puntoo_templates_1.playerJoinedSelfEmail)({
                statusLabel,
                level: booking.level,
                modality,
                organizerName,
            ...
            await this.mailService.sendTransactional({
                ...
                text: selfPack.text,
                html: selfPack.html,
                eventType: mail_constants_1.PuntooMailEvent.OPEN_MATCH_PLAYER_JOINED_SELF,
            });
        }
    }
    async notifyMatchLeaveMail(matchBefore, leftUserId, wasFullConfirmed) {
        if (!matchBefore.isMatch || matchBefore.visibility !== 'public')
            return;
        const modality = (0, puntoo_templates_1.matchGenderLabel)(this.matchGenderUiFromBooking(matchBefore.manualClubNotes));
        const organizerProfile = await this.prisma.profile.findUnique({
        ...
        if (selfMail) {
            const selfPack = (0, puntoo_templates_1.leaveSelfEmail)({
            ...
            await this.mailService.sendTransactional({
                ...
                text: selfPack.text,
                html: selfPack.html,
                eventType: mail_constants_1.PuntooMailEvent.OPEN_MATCH_LEAVE_SELF,
            });
        }
        if (wasFullConfirmed) {
            const remainingIds = matchBefore.participants
                ...
                .map((p) => p.profileId);
            const dropPack = (0, puntoo_templates_1.droppedFromFullEmail)({
            ...
            for (const pid of remainingIds) {
                ...
                await this.mailService.sendTransactional({
                    ...
                    text: dropPack.text,
                    html: dropPack.html,
                    eventType: mail_constants_1.PuntooMailEvent.OPEN_MATCH_LEAVE_FROM_FULL,
                });
            }
            return;
        }
        ...
        if (orgMail) {
            const orgPack = (0, puntoo_templates_1.leaveOrganizerEmail)({
                statusLabel: 'PENDIENTE',
                level: matchBefore.level,
                modality,
                organizerName,
            ...
            await this.mailService.sendTransactional({
                ...
                text: orgPack.text,
                html: orgPack.html,
                eventType: mail_constants_1.PuntooMailEvent.OPEN_MATCH_LEAVE_ORGANIZER,
            });
        }
    }
};
exports.ClubService = ClubService;
exports.ClubService = ClubService = ClubService_1 = __decorate([
