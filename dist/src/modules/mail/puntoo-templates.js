...
exports.formatEsDate = formatEsDate;
exports.formatEsTime = formatEsTime;
exports.formatMoneyEur = formatMoneyEur;
exports.matchGenderLabel = matchGenderLabel;
exports.bookingStatusLabel = bookingStatusLabel;
exports.closedBookingConfirmationEmail = closedBookingConfirmationEmail;
exports.openMatchPublishedEmail = openMatchPublishedEmail;
exports.playerJoinedOrganizerEmail = playerJoinedOrganizerEmail;
exports.playerJoinedSelfEmail = playerJoinedSelfEmail;
exports.matchConfirmedAllEmail = matchConfirmedAllEmail;
exports.leaveSelfEmail = leaveSelfEmail;
exports.leaveOrganizerEmail = leaveOrganizerEmail;
exports.droppedFromFullEmail = droppedFromFullEmail;
function esc(s) {
    return s
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
...
function bookingStatusLabel(status, occupiesSlot) {
    if (status === 'CONFIRMED' && occupiesSlot)
        return 'CONFIRMADO';
    return 'PENDIENTE';
}
function clubBlockText(club) {
    const contact = club.email && club.email.trim().length > 0 ? club.email.trim() : '—';
    return [
        `| Club: | ${club.name} |`,
...
function clubBlockHtml(club) {
    const contact = club.email && club.email.trim().length > 0 ? esc(club.email.trim()) : '—';
...
function participantRowsText(slots) {
    const lines = [];
...
function participantRowsHtml(slots) {
    return slots
        .map((s) => {
        const raw = s.name ?? '';
...
function closedBookingConfirmationEmail(input) {
...
function openMatchPublishedEmail(input) {
...
function playerJoinedOrganizerEmail(input) {
...
function playerJoinedSelfEmail(input) {
...
function matchConfirmedAllEmail(input) {
...
function leaveOrganizerEmail(input) {
...
function droppedFromFullEmail(input) {
...
//# sourceMappingURL=puntoo-templates.js.map