...
export function validateAlignedDateRange(
  alignedStartDate: string,
  alignedEndDate: string,
): string | null {
  const startDateParsed = parseYmd(alignedStartDate);
  if (!startDateParsed) return "La fecha de inicio no es valida";
  if (!alignedEndDate) return null;
  const endDateParsed = parseYmd(alignedEndDate);
